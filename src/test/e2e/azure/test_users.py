...
def test_an_address_returns_the_user_object(
    ...
    payload = json.loads(
        ok(
            runner.invoke(
                cli,
                ["-o", "json", "azure", "users", "get", "--user", "ann@example.com"],
            )
        ).stdout
    )
...
def test_a_display_name_uses_an_equality_filter(
    ...
    ok(runner.invoke(cli, ["azure", "users", "get", "--user", "Ann Example"]))
...
def test_an_unknown_address_exits_4(runner: Any, cli: Any, directory: Any) -> None:
    result = failed(
        runner.invoke(cli, ["azure", "users", "get", "--user", "nobody@example.com"]), 4
    )
