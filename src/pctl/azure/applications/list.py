...
@click.option("-n", "--limit", type=click.IntRange(min=1), help="Stop after N registrations.")
@click.option(
    "--page-size",
...
def command(
    ...
    """List application registrations, following Graph pagination.

    Only registrations owned by this tenant appear here. A third-party app in use but
    registered elsewhere has a service principal without a local application, so look
    for it under `pctl azure sp list` instead.

    \b
      pctl azure apps list
      pctl azure apps list --search "incident.io"
      pctl azure apps list --starts-with "Company " -o ndjson
      pctl azure apps list --count-only
    """
    from ..graph import DEFAULT_APPLICATION_SELECT, escape_odata, run
    ...
    fields = split_columns(select) or list(DEFAULT_APPLICATION_SELECT)
    table_columns = app.columns or (fields if select else DEFAULT_LIST_COLUMNS)

    async def _run() -> int:
        async with graph_client(app) as client:
            if count_only:
                click.echo(await client.count_applications(filter_expr=filter_expr))
                return -1
            renderer = Renderer(app.output, columns=table_columns)
            async for item in client.list_applications(
                select=fields,
    ...
    written = run(_run())
    if written >= 0:
        summarise(written, "app registration", quiet=app.quiet)
